 ...
 *   <li>{@link HttpAuth} — 根据 {@code marvis.anthropic.api-key/auth-token} 二选一</li>
...
public class HttpClientConfig {
     ...
     * 二选一构造 {@link HttpAuth}:
     * <ul>
     *   <li>{@code marvis.anthropic.api-key} 非空 → {@link ApiKeyAuth}</li>
     *   <li>否则 {@code marvis.anthropic.auth-token} 非空 → {@link BearerTokenAuth}</li>
     *   <li>都为空 → 启动失败,提示用户至少配一个</li>
     * </ul>
     */
    @Bean
    public HttpAuth httpAuth(MarvisProperties props) {
        ...
        throw new IllegalStateException(
                "marvis.anthropic.api-key or marvis.anthropic.auth-token must be set " +
     ...
     *
     * <p>Bean 名字明确为 {@code marvisObjectMapper},避免和 Spring Boot 4
     * 自动注册的 ObjectMapper 主 Bean 冲突(虽然实际配置一致,但显式声明更安全)。
     */
    @Bean(name = "marvisObjectMapper")
    @org.springframework.context.annotation.Primary
    public ObjectMapper marvisObjectMapper() {
     ...
     *
     * <p>注意参数 {@code ObjectMapper} 用 {@code marvisObjectMapper} 限定,
     * 避免与 Spring Boot 自带 jackson auto-config 的 mapper 冲突。
     */
    @Bean
    public AnthropicClient anthropicClient(OkHttpClient http,
                                           ObjectMapper marvisObjectMapper,
                                           MarvisProperties props,
                                           HttpAuth auth) {
        return new AnthropicHttpClient(http, marvisObjectMapper,
