 ...
 * 跟 {@link CronScheduler} 同模式。两个 tick 都跑在 {@code marvisTaskScheduler}
 * 共享调度池,Spring 容器接管启停。
 *
 ...
 *
 * <p>解法跟 {@link com.xilidou.marvis.tool.impl.TaskTool} 一样用 {@code @Lazy}:
...
public class CronQueueProcessor {
     ...
     */
    @Scheduled(
            fixedDelayString = "${marvis.cron.processor-tick-ms:200}",
            initialDelayString = "${marvis.cron.processor-initial-delay-ms:500}"
    )
    public void tick() {
