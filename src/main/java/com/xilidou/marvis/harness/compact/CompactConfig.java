 ...
 * <p>设计理由:
 * <ul>
 *   <li>不复用 {@code @ConfigurationProperties}:当前 marvis 主线代码不依赖 Spring
 *       容器({@link com.xilidou.marvis.harness.agent.AgentLoopHarness#fromEnv()}
 ...
 *       {@code @ConfigurationProperties("marvis.compact")} 不破坏现有调用</li>
 * </ul>
 *
 * <p>L1/L2 常量含义:
 * <ul>
...
public class CompactConfig {
    ...
    private static Path defaultTaskOutputDir() {
        // cwd 下 .task_outputs/tool-results/
        return Paths.get(System.getProperty("user.dir"), ".task_outputs", "tool-results");
    }

    private static Path defaultTranscriptDir() {
        // cwd 下 .transcripts/
