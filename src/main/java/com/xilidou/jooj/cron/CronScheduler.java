 ...
 * <ul>
 *   <li>tick 方法跑在 {@link com.xilidou.jooj.config.JoojExecutors#joojTaskScheduler 共享调度池}</li>
...
public class CronScheduler {
     ...
     * 的入队。
     *
     * <p>{@code fixedDelayString} 用 yaml 占位符直接读 {@code jooj.cron.scheduler-tick-ms},
     ...
     */
    @Scheduled(
            fixedDelayString = "${jooj.cron.scheduler-tick-ms:1000}",
            initialDelayString = "${jooj.cron.scheduler-initial-delay-ms:500}"
    )
    public void tick() {
        try {
