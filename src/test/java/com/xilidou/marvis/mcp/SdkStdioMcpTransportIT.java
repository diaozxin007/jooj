...
class SdkStdioMcpTransportIT {
    ...
    void setUp() {
        ...
        MarvisProperties.Mcp.Server everything = new MarvisProperties.Mcp.Server();
    ...
    @Test
    @DisplayName("callTool('echo', {message:'hello marvis'}) → 'Echo: hello marvis'")
    void call_echo_real_server() {
        // 先 listTools 触发启动
        transport.listTools("everything");

        String result = transport.callTool("everything", "echo",
                Map.of("message", "hello marvis"));
        assertNotNull(result);
        assertTrue(result.contains("hello marvis"),
